...
function stagingEnvironment(): Record<string, string> {
  ...
  return {
    ...
    ...Object.fromEntries(UAT_UXV2_IDENTITIES.flatMap((role, index) => [
      [`E2E_${role}_USERNAME`, `uat.uxv2.${role.toLocaleLowerCase("en-US").replace("_", ".")}`],
...
describe("UAT UXV2 fixture", () => {
  ...
  it("creates scoped primary and isolation identities and preserves password hashes on retry", () => {
    ...
    expect(twice).toEqual(once);
    expect(once.users).toHaveLength(UAT_UXV2_IDENTITIES.length);
