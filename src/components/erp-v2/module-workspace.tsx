...
export function ErpV2ModuleWorkspace(props: ErpV2ModuleWorkspaceProps) {
  ...
  if (!module || !props.visibleModuleIds.includes(props.moduleId)) {
    return <ErpShell user={props.user} activePath={props.activePath} title="Không có quyền truy cập">
      <section className="erp-v2-empty erp-v2-panel"><h1>Không có quyền truy cập</h1><p>Vai trò hiện tại không được phép mở module này.</p></section>
    </ErpShell>;
  }
  return <OperationsActorContext.Provider value={props.actor}>
    <ErpShell user={props.user} activePath={props.activePath} title={module.title}>
      ...
      {runtime.syncMeta.error ? <div className="erp-v2-workspace-alert error" role="alert"><AlertTriangle aria-hidden="true" /><span>{runtime.syncMeta.error}</span></div> : null}
