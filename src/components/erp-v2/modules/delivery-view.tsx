...
export function DeliveryView({
  ...
  const visibleJobs = focusedRecordId ? state.deliveryJobs.filter((job) => job.id === focusedRecordId) : state.deliveryJobs;
  return (
        ...
        <div className="panel-body">
          <DataTable
            ...
            rows={visibleJobs.map((job) => [
              <Link key="document" href={`/delivery/jobs/${job.id}`}>{job.documentNo}</Link>,
...
export function WorkerDeliveryView({
  ...
  const rows = (focusedRecordId ? state.deliveryJobs.filter((job) => job.id === focusedRecordId) : state.deliveryJobs).map((job) => {
