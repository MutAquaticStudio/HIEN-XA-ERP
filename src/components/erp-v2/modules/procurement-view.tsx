...
export function ProcurementView({
  ...
  const visibleOrders = focusedRecordId ? state.purchaseOrders.filter((item) => item.id === focusedRecordId) : state.purchaseOrders;
      ...
      <section className="panel">
        ...
        <div className="panel-body">
          <DataTable
            ...
            rows={visibleOrders.flatMap((order) =>
              order.lines.map((line) => {
                const linkedSale = linkedDirectSaleSummary(state, order, line);
                return [
                <Link key="document" href={`/procurement/orders/${order.id}`}>{order.documentNo} · {statusText(order.status)}</Link>,
...
export function WorkerProcurementView({
  ...
  const orders = focusedRecordId ? state.purchaseOrders.filter((order) => order.id === focusedRecordId) : state.purchaseOrders;
  const rows = orders.flatMap((order) => order.lines.map((line) => {
    ...
    return [
      <Link key="document" href={`/procurement/orders/${order.id}`}>{order.documentNo}</Link>,
      productLabel(state, line.productUnitId),
