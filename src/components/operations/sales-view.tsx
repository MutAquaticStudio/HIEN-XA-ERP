...
export function SalesView({ state, runOperation, createCommand, isPending }: { state: OperationsState; runOperation: OperationHandler; createCommand: CreateCommandHandler; isPending: boolean }) {
      ...
      {order ? <section className="panel">
        ...
        <div className="panel-body">
          <div className="summary-grid">
            ...
            <SummaryItem label="Cấp nguồn hàng" value={order.lines.every((line) => line.sourceType) ? "Đã cấp theo từng dòng" : "Chưa cấp nguồn"} />
          ...
          <DataTable headers={["Vật tư", "Số lượng", "Đã giao", "Nguồn hàng", "Thành tiền"]} rows={order.lines.map((line) => [productLabel(state, line.productUnitId), salesLineQuantityText(state, line), salesLineQuantityText(state, line, true), sourceText(line.sourceType), formatMoney(lineTotals(line).gross)])} />
          ...
          <DataTable headers={["Đơn bán", "Khách", "Ngày chứng từ", "Trạng thái", "Khách phải trả", "Hành động"]} rows={state.salesOrders.map((salesOrder) => [
            <strong key="document">{salesOrder.documentNo}</strong>, partyName(state, salesOrder.customerId), salesOrder.orderDate,
