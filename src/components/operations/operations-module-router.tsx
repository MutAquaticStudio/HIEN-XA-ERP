...
export function OperationsModuleRouter(props: OperationsModuleRouterProps) {
  const { activeModule, actor, state, syncMeta, visibleModuleIds, searchTerm, isPending, runOperation, createCommand, importWorkbook } = props;
  switch (activeModule) {
    case "overview":
    ...
    case "sales":
      return <SalesView state={state} runOperation={runOperation} createCommand={createCommand} isPending={isPending} />;
    case "procurement":
      return actor.role === "worker"
        ? <WorkerProcurementView state={state} runOperation={runOperation} isPending={isPending} />
        : <ProcurementView state={state} runOperation={runOperation} createCommand={createCommand} isPending={isPending} />;
    case "delivery":
      return actor.role === "worker"
        ? <WorkerDeliveryView state={state} runOperation={runOperation} isPending={isPending} />
        : <DeliveryView state={state} runOperation={runOperation} createCommand={createCommand} isPending={isPending} />;
    case "inventory":
