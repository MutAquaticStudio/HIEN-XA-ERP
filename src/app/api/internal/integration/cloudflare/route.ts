...
import {
  getCloudflareEnvironmentVariable,
  getCloudflareRequestBindings
...
export async function POST(request: Request) {
  const bindings = await getCloudflareRequestBindings();
  const deploymentStage = getCloudflareEnvironmentVariable(bindings.environment, "ERP_DEPLOYMENT_STAGE")?.toLocaleLowerCase("en-US");
  ...
  const productionHost = getCloudflareEnvironmentVariable(bindings.environment, "CLOUDFLARE_PRODUCTION_BASE_URL");
  if (productionHost) {
    try {
  ...
  const expectedSecret = getCloudflareEnvironmentVariable(bindings.environment, "CLOUDFLARE_INTEGRATION_SECRET") ?? "";
  ...
  try {
    ...
    const result = await runCloudflareIntegrationProbe(input.runId, bindings);
